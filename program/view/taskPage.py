...
class Ui_TaskPage(object):
    def setupUi(self, TaskPage):
        ...
        # Task list view (using QListWidget with checkboxes)
        ...
        self.taskList.setSelectionMode(QtWidgets.QAbstractItemView.MultiSelection)  # Allow multiple selections (checkboxes)
        
        # Add premade tasks (with checkboxes)
        premade_tasks = [
            "Task 1: Learn Python",
        ...
        try:
            with open('task_progress.pkl', 'rb') as file:
                ...
                for task, checkState in tasks:
                    item = QtWidgets.QListWidgetItem(task)
                    item.setCheckState(checkState)  # Load the saved check state
                    self.taskList.addItem(item)
